...
class ResolvedTicketDetailsBody extends StatefulWidget {
  const ResolvedTicketDetailsBody({Key? key}) : super(key: key);

  @override
  State<ResolvedTicketDetailsBody> createState() => _ProductBodyState();
}

class _ProductBodyState extends State<ResolvedTicketDetailsBody> {
  ...
  getResolvedTicketDetails() async {
    ...
    if (response.statusCode == 200) {
     ...
     setState(() {
       prefe.setString('technician_id',techId.toString());
     });
      print("______tech id is here__________$techId");
      print("$jsonResponse");
  ...
  addFeedBack() async {
    techId = getAllTicketDetails!.data?.technicianId.toString();
    ticketId = getAllTicketDetails!.data?.ticketId.toString();
    print('____________$addrating');
    SharedPreferences prefs  = await SharedPreferences.getInstance();
    String? userId =  prefs.getString(TokenString.userid);
    print("this is over tech id ____${techId}");
    print("${userId}");
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            const Text("Issues Image :",style: TextStyle(fontSize: 18,fontWeight: FontWeight.bold),),
            const SizedBox(height: 20,),
            Image.network("https://cubixsys.com/cubixsys-support/${getAllTicketDetails!.data!.reasonsImage}",),
            const SizedBox(height: 30,),
            const Text("Issues Description :",style: TextStyle(fontSize: 18,fontWeight: FontWeight.bold),),
            SizedBox(height: 10,),
            Text("${getAllTicketDetails!.data!.reasonsDescription}",style: TextStyle(fontWeight: FontWeight.bold,fontSize: 16),),
              ...
              children: [
                ...
                InkWell(
                  ...
                  child: Container(
                    ...
                    child: const Center(child: Text("Feedback",style: TextStyle(color: Colors.white,fontWeight: FontWeight.bold,fontSize: 18),)),
                  ),
                ),
