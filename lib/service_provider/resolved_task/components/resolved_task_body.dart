...
import '../resolved_task_details.dart';

class ResolvedTaskBody extends StatefulWidget {
  const ResolvedTaskBody({Key? key}) : super(key: key);

  @override
  State<ResolvedTaskBody> createState() => _ProductBodyState();
}

class _ProductBodyState extends State<ResolvedTaskBody> {

  String? taskId;


  TotalTask? resolveTaskList;

  resolveTask() async {
    SharedPreferences prefs  = await SharedPreferences.getInstance();
    String? userId =  prefs.getString(TokenString.userid);
    var request = http.MultipartRequest('POST', Uri.parse('https://cubixsys.com/cubixsys-support/api/ticketList'));
    ...
    if (response.statusCode == 200) {
      ...
      final jsonResponse = TotalTask.fromJson(jsonDecode(finalResponse));
      print("$jsonResponse");
      print("__overprotects$finalResponse");
      setState(() {
        resolveTaskList = jsonResponse;
      });
    }
    else {
  ...
  Widget build(BuildContext context) {
    ...
    return resolveTaskList == null ? const Center(child: CircularProgressIndicator(
      semanticsLabel: 'Circular progress indicator',
    ),) :resolveTaskList!.data!.isEmpty?const Center(child:Text("No data to show",style: TextStyle(fontSize: 20),),) :
    ListView.builder(
              ...
              Card(
                ...
                child: Row(
                  children: [
                    ...
                    Column(
                      ...
                      children: [
                        Padding(
                          ...
                          child: Text("${resolveTaskList!.data![index].ticket}",style: TextStyle(fontSize: 18,fontWeight: FontWeight.bold),),
                        ),
                        SizedBox(
                          ...
                          child:  ListTile(
                            title: Padding(
                              ...
                              child: Text("${resolveTaskList!.data![index].productName} ", style: TextStyle(fontWeight: FontWeight.w400, fontSize: 20 ), ),
                            ),
                            subtitle: Padding(
                              padding: EdgeInsets.only(bottom: 8.0),
                              child: Text("${resolveTaskList!.data![index].city}\n${resolveTaskList!.data![index].state}", style: TextStyle(fontWeight: FontWeight.w500, fontSize: 15 ),),
                            ),
                          ),
                        ),
                      ],
                    ...
                    InkWell(
                      onTap: (){
                        print('${resolveTaskList!.data![index].taskId}');
                        Navigator.push(context, MaterialPageRoute(builder: (context)=> const ResolvedTaskDetails(
                        )));
      ...
      }, itemCount: resolveTaskList?.data?.length,);
  }
}
